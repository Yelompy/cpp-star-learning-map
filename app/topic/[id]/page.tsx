...
import { topicExpansions } from "@/lib/topic-expansions";
...
export default async function TopicPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (index < 0 || !deepDives[id] || !topicExpansions[id]) notFound();
  const node = knowledgeNodes[index];
  const stage = stages.find((item) => item.id === node.stageId);
  if (!stage) notFound();
  return <TopicLesson node={node} stage={stage} deepDive={deepDives[id]} expansion={topicExpansions[id]} previous={knowledgeNodes[index - 1]} next={knowledgeNodes[index + 1]} />;
}
